...
def match_candidate(candidate, job):
    ...
    missing_preferred = preferred_skills.difference(candidate_skills)

    candidate_experience = candidate["years_experience"]
    minimum_experience = job["minimum_experience"]

    experience_met = candidate_experience >= minimum_experience
...
if __name__ == "__main__":
    resume_path = "data/sample_resumes/alice_resume.txt"
    job_path = "data/sample_jobs/data_analyst_job.txt"

    resume_text = read_resume(resume_path)
    job_text = read_resume(job_path)
